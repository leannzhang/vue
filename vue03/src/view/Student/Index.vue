<template>
  <div>
    ...
    <el-table
      border
      style="width: 100%"
      :data="StudentInfo.slice((queryInfo.pagenum-1)*queryInfo.pagesize,queryInfo.pagenum*queryInfo.pagesize)"
      stripe
    >
      ...
      <el-table-column label="操作">
        ...
        <template slot-scope="scope">
          ...
          ></el-button>
          <!--删除按钮-->
          <el-button type="danger" icon="el-icon-delete" size="mini" @click='removeStudent(scope.row.Acount)'></el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
      ...
      axios.get('/api/StudentInfo', {
        ...
        .then(res => {
          if (res.data.state === 1) {
            ...
            this.total = this.StudentInfo.length
          } else {
            return false
          }
        })
    ...
    selectStudent(Acount) {
      axios.get('/api/selectStudent', {
        ...
        .then(res => {
          if (res.data.state === 1) {
            console.log(res.data.selectStudentInfo)
            this.editForm = res.data.selectStudentInfo[0]
            // this.editForm = res.data.selectStudentInfo.RowDataPacket
          } else {
            return false
          }
        })
    ...
    // 修改教师数据
    editStudentInfo(Acount) {
      axios.get('/api/editStudentInfo', {
        params: {
    ...
    // 删除教师
    deleteStudent (Acount) {
      axios.get('/api/deleteStudent', {
        params: {
          Acount
        }
      })
