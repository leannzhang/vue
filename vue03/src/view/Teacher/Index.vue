<template>
  <div>
    ...
    <el-table
      border
      style="width: 100%"
      :data="TeacherInfo.slice((queryInfo.pagenum-1)*queryInfo.pagesize,queryInfo.pagenum*queryInfo.pagesize)"
      stripe
    >
      ...
      <el-table-column label="操作">
        ...
        <template slot-scope="scope">
          ...
          ></el-button>
          <!--删除按钮-->
          <el-button type="danger" icon="el-icon-delete" size="mini" @click='removeTeacher(scope.row.Acount)'></el-button>
        </template>
      </el-table-column>
    </el-table>
  ...
  methods: {
      ...
      axios.get('/api/TeacherInfo', {
        ...
        .then(res => {
          if (res.data.state === 1) {
            ...
            this.total = this.TeacherInfo.length
          } else {
            return false
          }
        })
    ...
    selectTeacher(Acount) {
      axios.get('/api/selectTeacher', {
        ...
        .then(res => {
          if (res.data.state === 1) {
            console.log(res.data.selectTeacherInfo)
            this.editForm = res.data.selectTeacherInfo[0]
            // this.editForm = res.data.selectTeacherInfo.RowDataPacket
          } else {
            return false
          }
        })
    ...
    // 修改教师数据
    editTeacherInfo(Acount) {
      axios.get('/api/editTeacherInfo', {
        params: {
